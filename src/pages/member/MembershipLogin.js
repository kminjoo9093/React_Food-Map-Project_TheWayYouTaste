...
function MembershipLogin({ setIsLoggedIn, setUser }) {
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      alert(
        "회원가입 완료! 로그인 해주세요. 닉네임은 내 정보에서 변경 가능합니다. ",
      );
      setActive(true);
  ...
  return (
    <div className={style.loginWrap}>
      <div className={`${style.container} ${active ? style.active : ""}`}>
        <div
          className={`${style.formBox} ${style.registerFormBox} ${style.login}`}
        >
          ...
          <div className={style.mobToggleBox}>
            <div className={style.mobToggleInner}>
              <h1 className={style.toggleTitle}>
                Signing up only takes
                <br></br>a few seconds.
              </h1>
        ...
        {/* 로그인 */}
        <div
          className={`${style.formBox} ${style.mobLoginWrap} ${style.register}`}
        >
          {/* 모바일 토글박스 */}
          <div className={style.mobToggleBox}>
            <div className={style.mobToggleInner}>
