...
function SearchStore() {
  ...
  // 현재 위치 기반 초기 로드 - 훅 내부의 storeList가 변경될 때마다 반영
  useEffect(() => {
    ...
    const initLoad = async () => {
      ...
      setRegion(
        mapQueryToRegion({
          ...
          sggCode: sgg,
          dongCode: dong,
          sidoName: paramDoName,
          sggName: paramSiName,
          dongName: paramDongName,
        }),
      );
      ...
      if (keyword) {
        list = await GetStoreList(
          `${SERVER_URL}/youtaste/search/store/keyword?name=${keyword}`,
        );
      }
      //메인페이지 필터 설정
      ...
      if (list.length > 0) {
        ...
        if (urlCategoryArray.length > 0) {
          filteredResult = list.filter((record) =>
            urlCategoryArray.includes(record.storeCatName),
          );
        }
  ...
  // 범위 내 재검색 함수
  const displayViewPortMarkers = useCallback(
    async (area) => {
