...
function SearchStore({storeCategories, sidoList}){
    ...
    const location  = useLocation(); // 검색어 
    // URL의 쿼리 스트링 해석 (예: ?keyword=치킨)
    ...
    useEffect(() => {
        const initLoad = async () => {
            if (keyword) {
                // 1. 검색어가 있을 때: 키워드 API 호출
                console.log("검색어 기반 조회 시작:", keyword);
                try {
