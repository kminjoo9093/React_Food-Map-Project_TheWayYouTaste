...
- **문제**
  - 기존 Zustand 및 로컬 상태를 중심으로 검색 조건을 관리하여 새로고침 시 검색 상태가 유지되지 않고,<br/>
    실제 검색 조건과 UI선택 상태 불일치하는 문제 발생 
    <br/>
    
- **해결**
  -  URL Query Parameter를 실제 검색 상태로 재설계하고, Zustand 기반 전역 상태(지역/카테고리 필터)는 UI 선택 상태로 역할을 분리
    <br/>
    
- **결과**
  - 새로고침 및 페이지 재진입 시 검색 조건 복원
  - URL 공유를 통한 동일 검색 결과 재현 가능
  - 실제 검색 조건과 UI 선택 상태 간 불일치 방지

<br><br>

#### **2. 데이터 검색 범위가 제한되는 문제를 해결하기 위해 ‘범위 내 재검색’ 버튼 추가로 데이터 호출 메커니즘 전환** &emsp; 
...
- **해결**
  - [**‘범위 내 재검색' 버튼**을 데이터 호출 파라미터 전환 트리거로 활용](./src/pages/search/SearchStore.js)하여  <br/>
  ...
  - searchMode, isMoved 상태를 통해 지역 선택 모드/지도 이동 모드 구분으로 검색 컨텍스트에 맞는 UI 제공
    <br/>
...
#### **3. 지역 코드 패턴 분석 기반의 파싱 로직 재설계로 데이터 누락 0% 의 무결성 확보**
